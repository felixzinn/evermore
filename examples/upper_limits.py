...
# fix mu to 0
model_fixed_mu = eqx.tree_at(lambda t: t.mu.value, model, jnp.array(0))
...
@jax.jit
def unconditional_fit(steps: int = 1000) -> tuple[eqx.Module, tuple]:
...
@jax.jit
def conditional_fit(steps: int = 1000) -> tuple[eqx.Module, tuple]:
    def fit(step, model_optstate):
        model, opt_state = model_optstate
...
def likelihood_ratio() -> tuple[float, Array]:
    """
    Likelihood ratio
    ...
    model_conditional, _ = conditional_fit()
